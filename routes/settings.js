  ...
  router.post('/settings/integrations/test', async (req, res) => {
    ...
    try {
      ...
      if (source === 'maintainx') {
        ...
        if (orgId) {
          baseHeaders['X-MX-Organization-Id'] = String(orgId);
          baseHeaders['X-Organization-Id']    = String(orgId);
          baseHeaders['Organization-Id']      = String(orgId);
        }
        // Org context is via headers only (organizationId query param is rejected).
        const candidates = [
