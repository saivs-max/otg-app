...
module.exports = (db) => {
  ...
  router.patch('/workorders/:id', (req, res) => {
    ...
    const fields = ['status','store_name','store_id','store_address','cart_count','scheduled_date','description'];
    const updates = [];
    const params  = [];
...
async function fetchFromSource(source, ticketId, db) {
  ...
  if (source === 'maintainx') {
    ...
    const headersWithOrg = { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' };
    if (orgId) {
    ...
    // Try with org headers first; if response is empty, retry without — sometimes
    // org-scoped tokens reject globally-addressable WO IDs when org is forced.
    const attempts = orgId
      ? [{ label: 'with org headers',    headers: headersWithOrg },
         { label: 'without org headers', headers: headersNoOrg }]
    ...
    // that's a cart SERIAL not a count, so don't infer count from it.
    const cart_count =
