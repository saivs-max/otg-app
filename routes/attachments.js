...
module.exports = (db) => {
  ...
  router.get('/attachments/:id/download', (req, res) => {
    // v0.35 — auth middleware populates x-user-id from the bearer token
    // (or ?token= query for raw <a href> links). The bare ?uid= bypass is gone.
    const userId = Number(req.header('x-user-id'));
    ...
    if (att.user_id !== userId) return res.status(403).send('not yours');
