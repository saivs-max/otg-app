...
async function renderHome(root) {
  const [actives, wos, drafts] = await Promise.all([
...
async function renderAdd(root) {
  ...
  let selected = { category: 'mileage', subcategory: '', work_order_id: open[0]?.id || '', expense_date: defaultDate };
  ...
  function html() {
    ...
    return `
      ...
      <input class="field" id="woSearch" placeholder="🔎 Search by ID, store, type…" style="margin-bottom:6px;" />
      <select class="field" id="woSel" size="${Math.min(5, Math.max(2, open.length))}">
        ${open.map(woOption).join('')}
      </select>
      ...
      ${cat === 'mileage' ? `
        ...
        <input class="field" type="number" step="0.1" min="0" id="qtyInp" placeholder="32.4" />
      ...
      ` : `
        ...
        <input class="field" type="number" step="0.01" min="0" id="amtInp" placeholder="0.00" />
      ...
      <input class="field" id="descInp" placeholder="e.g., Edgewater on-site mileage" />

      <span class="label">Receipt (optional but recommended)</span>
      <div id="recBlock"></div>

      <button class="btn btn-primary btn-block" id="saveBtn">Save to invoice</button>
    `;
  }
  ...
  function bind() {
    ...
    $$('#subChips .chip').forEach(c => c.addEventListener('click', () => {
      selected.subcategory = c.dataset.sub; root.innerHTML = html(); bind();
    }));
    $('#woSearch')?.addEventListener('input', e => {
      const q = e.target.value.toLowerCase();
    ...
    $('#woSel').addEventListener('change', e => { selected.work_order_id = e.target.value; root.innerHTML = html(); bind(); });
    ...
    if (recBlock) {
      if (pendingReceipt) {
        ...
        `;
        $('#rmRec').addEventListener('click', () => { pendingReceipt = null; root.innerHTML = html(); bind(); });
      } else {
        makeReceiptPicker(recBlock, {
          label: '📷 Take photo or pick file',
          onFile: (payload) => {
            pendingReceipt = payload;
            // Re-render to show the queued receipt; keep selected state
            root.innerHTML = html(); bind();
          },
        });
      }
    ...
    $('#saveBtn').addEventListener('click', async () => {
      const body = {
        ...
        description: $('#descInp').value || undefined,
      };
      if (selected.category === 'other') {
        if (!selected.subcategory) return toast('Pick a sub-option (Meal, Tools, Hotel, …)', 'err');
        body.subcategory = selected.subcategory;
      }
      if (selected.category === 'mileage') {
        body.quantity = Number($('#qtyInp')?.value);
        if (!body.quantity) return toast('Enter miles', 'err');
      } else {
        body.amount = Number($('#amtInp')?.value);
      ...
      try {
        ...
        // If we were pinned to a specific invoice, return there. Otherwise go
        // to the current week's invoice as before.
        if (pinnedId) {
          const dest = pinnedId;
    ...
    <div class="flex gap-12">
      ...
      <div style="flex:1;">
        ...
        <select class="field" id="te_mode">
          <option value="work"  ${(t.mode||'work')==='work'?'selected':''}>Work (billable)</option>
          <option value="drive" ${t.mode==='drive'?'selected':''}>Drive (non-billable)</option>
        </select>
      </div>
    </div>
...
function openEditExpenseSheet(exp) {
  ...
  showSheet(`
    ...
    <select class="field" id="ecat">
      ${['mileage','tolls','parking','meals','tools','vendor','other']
        .map(c => `<option value="${c}" ${c===exp.category?'selected':''}>${capitalize(c)}</option>`).join('')}
    </select>
...
function renderAmtBlock(cat, exp) {
  ...
  return `<span class="label">Amount ($)</span>
      <input class="field" id="eamt" type="number" step="0.01" min="0" value="${exp.category!=='mileage'?(exp.amount||''):''}" placeholder="0.00" />`;
}
...
async function renderApprovalQueue(root) {
  ...
  const flagged = queue.filter(i => (i.notes || '').length > 0).length;
  ...
  root.innerHTML = `
    ...
    <div class="card" style="background: var(--ic-green-deep); color: #fff; border: 0;">
      <div class="flex between" style="align-items: center;">
        ...
        <div style="text-align: right;">
          ...
          ${flagged ? `<div style="font-size: 11px; color: #ffc89a; margin-top: 4px;">⚠ ${flagged} with notes</div>` : ''}
        </div>
      </div>
    </div>
    ...
    <div class="card-grid">
      ${queue.map(inv => {
        ...
        return `
          <div class="card tap" data-inv="${inv.id}" style="${aging ? 'border-left: 4px solid var(--ic-orange);' : ''}">
            <div class="flex between" style="align-items: flex-start;">
              <div style="flex: 1; min-width: 0;">
...
async function renderDashboard(root) {
  ...
  root.innerHTML = `
    ...
    <div class="dash-grid-2">
      ...
      ${renderBarCard('Top stores by spend', r.by_store.slice(0, 10), 'store_name', 'total', maxOf(r.by_store, 'total'),
        (s) => `${s.wo_count} ${s.wo_count===1?'visit':'visits'} · avg ${fmt$(s.total / Math.max(1, s.wo_count))}/visit`,
        { drillKey: 'store', drillValue: (s) => s.store_name })}
    </div>
  ...
  $$('.bar-row[data-drill], .stack-row[data-drill], .donut-svg [data-drill], .donut-legend-row[data-drill], .legend-pill[data-drill]').forEach(el => {
...
async function renderMine(root) {
  ...
  function html() {
    ...
    return `
      ...
      <!-- + New Invoice (for past week) + Upload PDF -->
      <div class="flex gap-12" style="margin: 14px 0;">
        <button class="btn btn-warn" style="flex:1;" id="newInvBtn">＋ New invoice (past week)</button>
        <button class="btn btn-primary" style="flex:1;" id="techUploadInvBtn">📄 Upload an old invoice</button>
      </div>
...
async function renderInvoiceDetail(root, invoiceId) {
  ...
  // prev/next ordering — newest first in the list, so prev = newer, next = older
  ...
  root.innerHTML = `
    ...
    ${list.length > 1 ? `
      <div class="flex between" style="margin-bottom:8px;">
        ...
        <span style="font-size:11px; color:var(--muted)">${idx + 1} of ${list.length}</span>
    ...
    <div class="invoice-doc"${invoice.invoice_type === 'vendor' ? ' style="display:none;"' : ''}>
      ...
      <div class="inv-billto-row">
        <div class="inv-billto">
          ...
          <div>Cart Tech Operations — AP</div>
          <div>50 Beale St</div>
    ...
    ${invoice.status === 'draft' && extracted && (isManagerProxy || isManagerView) ? renderExtractedPanel(invoice, extracted, extracted_at, summary) : ''}
