...
// v0.53 — disable FK enforcement during the truncate so leftover rows in
// auxiliary tables (sessions, launch_actuals, cost_tracker_overrides) can't
// block a fresh re-run with a UNIQUE constraint error on users.username.
...
].forEach(t => { try { db.exec(`DELETE FROM ${t}`); } catch (e) { console.warn(`[seed-demo] skip ${t}: ${e.message}`); } });
...
// v0.60 — Seed default corp-card categories. opsId/srId can both add more
// from the Corp Card tab; we seed a representative starter set here so the
// demo has something to render without manual setup.
const insCorpCat = db.prepare("INSERT OR IGNORE INTO corp_card_categories (name, created_by) VALUES (?, ?)");
const corpCatIds = {};
for (const c of ['Travel', 'Hotel', 'Events', 'Meals', 'Software', 'Tools', 'Other']) {
  insCorpCat.run(c, opsId);
  corpCatIds[c] = db.prepare("SELECT id FROM corp_card_categories WHERE name = ?").get(c).id;
}
...
// ---------- v0.60 — sample corp-card spend for the demo dashboard ----------
console.log('Seeding sample corp-card expenses…');
{
  const insCC = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  // Pull a few open WOs to attach corp-card spend to (for store linkage).
  const openWos = db.prepare("SELECT id, store_name FROM work_orders WHERE status IN ('open','in_progress','done') ORDER BY id DESC LIMIT 20").all();
  const samples = [
    // Ops Mgr (Maitland) charges for Aramiwale's NJ launch hotel + travel
    { by: opsId, tech: tech1, cat: 'Hotel',   amt: 348.92, days: 5,  desc: 'Marriott Edgewater — 2 nights for NJ launch' },
    { by: opsId, tech: tech1, cat: 'Travel',  amt: 142.50, days: 6,  desc: 'Airfare LAX→EWR' },
    { by: opsId, tech: tech2, cat: 'Hotel',   amt: 412.18, days: 8,  desc: 'Hampton Inn Hackensack — 3 nights' },
    { by: opsId, tech: tech2, cat: 'Travel',  amt: 78.40,  days: 9,  desc: 'Uber EWR airport to site' },
    { by: opsId, tech: tech3, cat: 'Meals',   amt: 64.27,  days: 3,  desc: 'Team dinner — Bridge & Harbison launch night' },
    { by: opsId, tech: tech1, cat: 'Tools',   amt: 89.99,  days: 11, desc: 'Replacement drill bits — emergency' },
    // Sr Mgr (Reshmi) charges for events and software (no specific tech)
    { by: srId,  tech: null, cat: 'Events',   amt: 1240.00, days: 14, desc: 'Q2 ops team offsite — venue deposit' },
    { by: srId,  tech: null, cat: 'Software', amt: 299.00,  days: 21, desc: 'Annual seat: route-planning SaaS' },
    { by: srId,  tech: null, cat: 'Events',   amt: 685.00,  days: 7,  desc: 'Field-tech recognition dinner' },
    { by: srId,  tech: tech3, cat: 'Travel',  amt: 218.00,  days: 4,  desc: 'Priya training conference travel' },
  ];
  for (const s of samples) {
    const wo = s.tech ? openWos[range(0, openWos.length - 1)] : null;
    insCC.run(
      s.by, s.tech, wo?.id || null, wo?.store_name || null,
      corpCatIds[s.cat], iso(daysAgo(s.days)), s.amt, s.desc
    );
  }
}
