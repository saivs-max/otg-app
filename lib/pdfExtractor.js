...
function extractLaborLines(text, lines) {
  ...
  // v1.06 — tolerate the app-generated (PDFKit) layout: pdf-parse glues the date
  // cell to the row's details/amount, e.g. "13-Apr[4/13] [Maintenance] MaintainX
  // #16" and empty days "14-Apr0.00$40.00$0.00". No `\b` after the month (there is
  // no word boundary between "Apr" and a following digit). The (?![A-Za-z\s])
  // guard requires the month to be immediately followed by NON-letter, NON-space
  // (or end-of-line): both real layouts glue with no space after the month, while
  // a free-text note like "15-May reschedule requested" has a space and is
  // correctly treated as content — not a day header that would re-date the next
  // row. Group 3 captures the glued remainder, fed back onto the row buffer. A
  // date alone on its own line (Brennan/Sbot template) still matches (empty
  // remainder), so that path is unchanged.
  const dayRe = /^(\d{1,2})-([A-Za-z]{3})(?![A-Za-z\s])(.*)$/;
  ...
  for (const raw of lines) {
    ...
    if (dm) {
      // v1.06 — only switch days when the token is a REAL month. normalizeDate
      // returns null otherwise, so a content line that merely starts "NN-Xyz"
      // can't blank currentDate and silently drop the rows that follow it.
      const nd = normalizeDate(`${dm[1]}-${dm[2]}`);
      if (nd) {
        flush();
        currentDate = nd;
        buf = [];
        // Feed the glued remainder (details/amount) back onto the buffer so the
        // existing timeRe/emptyRe + description/ticket logic fires. Empty for the
        // date-alone template → identical behavior to before.
        const rem = (dm[3] || '').trim();
        if (rem) buf.push(rem);
        continue;
      }
...
// v0.96 — handles two layouts:
//   (A) Two-line (Brennan/Sbot template): label on its own line, value on next.
//   (B) Inline (app-generated PDFs via PDFKit): PDFKit draws the label and the
//       $ amount at the same Y coordinate; pdf-parse merges them into one line,
//       e.g. "TOTAL$184.00" or "SUBTOTAL (Labor)$184.00". The label-only regex
//       /^TOTAL$/i never matched, so totals came back null → $0 displayed.
//       Guard all assignments with === null so the first real match wins and a
//       later "TOTAL MILEAGE & REIMBURSEMENT…" line can't clobber a good value.
function extractTotals(text, lines) {
  ...
  // Inline-match helper: label + $ value on the same line.
  // Accepts "$184.00", "$184", or bare "184.00" (with cents).
  function inlineMoney(line, labelRe) {
    const m = line.match(new RegExp(labelRe.source + /[^$]*\$\s*([\d,]+(?:\.\d{2})?)/.source, 'i'));
    return m ? Number(m[1].replace(/,/g, '')) : null;
  }
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Layout A: label-only line, value on a following line.
    if (/^SUBTOTAL\b/i.test(line) && out.subtotal       === null) out.subtotal       = moneyAfter(lines, i);
    if (/^Mileage$/i.test(line)   && out.mileage_amount === null) out.mileage_amount = moneyAfter(lines, i);
    if (/^Other$/i.test(line)     && out.other          === null) out.other          = moneyAfter(lines, i);
    if (/^TOTAL$/i.test(line)     && out.total          === null) out.total          = moneyAfter(lines, i);
    // Layout B: label and $ value merged onto the same line by pdf-parse.
    if (out.subtotal       === null) { const v = inlineMoney(line, /^SUBTOTAL\b[^$]*/); if (v !== null) out.subtotal       = v; }
    if (out.mileage_amount === null) { const v = inlineMoney(line, /^Mileage/);         if (v !== null) out.mileage_amount = v; }
    if (out.other          === null) { const v = inlineMoney(line, /^Other/);           if (v !== null) out.other          = v; }
    // "^TOTAL$" only (no trailing words) so "TOTAL MILEAGE & REIMBURSEMENT…" can't match.
    if (out.total          === null) { const v = inlineMoney(line, /^TOTAL(?!\s+\w)/);  if (v !== null) out.total          = v; }
