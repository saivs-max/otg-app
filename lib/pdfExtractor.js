...
async function extractFromPdfBuffer(buf) {
  ...
  return { ok: true, text, info: parsed.info, pages: parsed.numpages, summary };
}
...
function extractTotals(text, lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (/^SUBTOTAL$/i.test(line)) out.subtotal = parseDollar(lines[i + 1]);
    if (/^Mileage$/i.test(line))  out.mileage_amount = parseDollar(lines[i + 1]);
    if (/^Other$/i.test(line))    out.other = parseDollar(lines[i + 1]);
    if (/^TOTAL$/i.test(line))    out.total = parseDollar(lines[i + 1]);
