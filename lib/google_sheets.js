...
async function pushDashboardToSheet(payload, costRows = []) {
  ...
  // v0.67 — grand-total row mirrors the in-app footer (Labor / Travel / Expenses
  // / 3P / Total). Pending rows carry $0, so the sums equal approved actuals.
  const ctmTotalRows = [];
  if (costRows.length) {
    const first = 2, last = costRows.length + 1;
    const row = new Array(ctmHeader.length).fill('');
    row[1]  = 'TOTAL (approved actuals)';     // B  Store
    row[10] = `=SUM(K${first}:K${last})`;     // K  Actual Labor
    row[11] = `=SUM(L${first}:L${last})`;     // L  Actual Travel
    row[12] = `=SUM(M${first}:M${last})`;     // M  Actual Expenses
    row[15] = `=SUM(P${first}:P${last})`;     // P  Actual 3P Cost
    row[16] = `=SUM(Q${first}:Q${last})`;     // Q  Actual Total
    ctmTotalRows.push(row);
  }

  // ----- DASHBOARD (approved actuals only, broken out by work type) -----
  // v0.67 — mirror the in-app dashboard: one column per work type (not just
  // Deployment/Retrofit/Other). Aggregation matches aggregateCostTrackerByMonth.
  const months = ['January','February','March','April','May','June','July',
                  'August','September','October','November','December'];
  const present = new Set();
  for (const r of costRows) { if (!r.pending_approval) present.add(r.service_type || 'Other'); }
  const PREFERRED = ['Deployment','Retrofit','Maintenance','Repair'];
  let types = [...PREFERRED.filter(t => present.has(t)), ...[...present].filter(t => !PREFERRED.includes(t)).sort()];
  if (!types.length) types = ['Deployment'];
  const colL = (i) => { let s = ''; i += 1; while (i > 0) { const m = (i - 1) % 26; s = String.fromCharCode(65 + m) + s; i = Math.floor((i - 1) / 26); } return s; };
  const NT = types.length, totalColIdx = NT + 1, countColIdx = NT + 2;
  const byMonth = {};
  for (const r of costRows) {
    if (r.pending_approval) continue;          // approved + completed actuals only
    const m = r.service_month || 'Unscheduled', t = r.service_type || 'Other';
    byMonth[m] = byMonth[m] || { byType: {}, Count: 0 };
    byMonth[m].byType[t] = (byMonth[m].byType[t] || 0) + (r.actual_total || 0);
    byMonth[m].Count += 1;
  }
  const monthsWithData = months.filter(m => byMonth[m]);
  if (byMonth['Unscheduled']) monthsWithData.push('Unscheduled');   // never drop rows
  const dashRows = [
  ...
  monthsWithData.forEach((m, i) => {
    ...
    dashRows.push([
      m,
      ...types.map(t => +(+(v.byType[t] || 0)).toFixed(2)),
      `=SUM(${colL(1)}${xr}:${colL(NT)}${xr})`,
      v.Count,
    ]);
  });
  if (monthsWithData.length) {
    ...
    const row = ['Total'];
    for (let c = 1; c <= countColIdx; c++) row.push(`=SUM(${colL(c)}4:${colL(c)}${totalRow - 1})`);
    dashRows.push(row);
  }

  const tabs = {
    'COST TRACKER MAIN': [ctmHeader, ...ctmDataRows, ...ctmTotalRows],
    'DASHBOARD':         dashRows,
  };
