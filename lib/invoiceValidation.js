...
function validateInvoice({ kind, extraction, text }) {
  ...
  if (kind === 'vendor') {
    ...
    // v1.06 — a contractor invoice that parsed a positive total but ZERO labor
    // line items would import as $0 (computeInvoice sums imported rows, not the
    // parsed total). Flag that contradiction as CRITICAL so it is held for review
    // instead of silently posting $0. A genuinely empty/zero invoice stays a warn.
    if (labor.length === 0) {
      if (total != null && total > 0)
        add('critical', 'no_line_items_but_total',
          `Invoice total ${total} was parsed but no labor line items were extracted — importing would post $0.`);
      else
        add('warn', 'no_line_items', 'No labor line items extracted.');
    }
