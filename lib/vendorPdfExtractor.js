...
const BUYER_PATTERNS = [
  /MAPLEBEAR/i, /INSTACART/i, /CART\s*TECH\s*OPS/i,
];
...
function findVendorName(text, lines) {
  ...
  // Strategy C: scan the first ~25 lines for a company-name-shaped line —
  // mostly upper-case, contains a word like Inc / LLC / Corp / Co /
  // Equipment / Services / Technologies / Systems, and isn't us.
  const COMPANY_HINTS = /(LLC|INC\.?|CORP\.?|CO\.?|LTD\.?|PLLC|LLP|EQUIPMENT|SERVICES?|TECHNOLOGIES|SYSTEMS|SOLUTIONS|ENTERPRISES|GROUP|COMPANY|PARTNERS|HOLDINGS|INDUSTRIES|MANUFACTURING|SUPPLY|PRODUCTS|ASSOCIATES)\b/i;
  for (const ln of lines.slice(0, 25)) {
    if (ln.length < 4 || ln.length > 80) continue;
    if (isBuyer(ln)) continue;
    if (!COMPANY_HINTS.test(ln)) continue;
    // Must be mostly letters/spaces (avoid address lines with lots of digits)
    if (((ln.match(/\d/g) || []).length) > 3) continue;
    return cleanCompanyName(ln);
  }
