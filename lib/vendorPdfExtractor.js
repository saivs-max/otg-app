// lib/vendorPdfExtractor.js — v0.80
//
...
function findLineItems(text, lines) {
  ...
  // Shape T — Truno "SERVICE INVOICE" grid (v0.80). pdf-parse splits the five
  // money columns (Labor / Travel / Materials / Other / Total) across three lines:
  //   Line A: "<equipment><serial>$<labor> $<travel> $<materials>"  (3 amounts)
  //   Line B: "<make/model>"                                         (optional text)
  //   Line C: "$<other> $<total>"                                    (2 amounts)
  // We anchor on Line A (equipment code + 3 amounts), pair it with Line C within
  // the next 3 lines, validate labor+travel+materials+other ≈ total, then emit
  // ONE line item per non-zero category — giving the per-cost-type breakdown
  // that TRUNO invoices carry (not just a lump total).
  add((() => {
    const three = /^(.*?)\$\s*([\d,]+\.\d{2})\s+\$\s*([\d,]+\.\d{2})\s+\$\s*([\d,]+\.\d{2})\s*$/;
    const two   = /^\$\s*([\d,]+\.\d{2})\s+\$\s*([\d,]+\.\d{2})\s*$/;
    const CATS  = ['Labor', 'Travel', 'Materials', 'Other'];
    const out   = [];
    for (let i = 0; i < lines.length; i++) {
      ...
      const labor     = parseDollar(m[2]) || 0;
      const travel    = parseDollar(m[3]) || 0;
      const materials = parseDollar(m[4]) || 0;
      let desc = null, total = null, other = 0;
      for (let j = i + 1; j < Math.min(i + 4, lines.length); j++) {
        const t = lines[j].match(two);
        if (t) {
          const oth = parseDollar(t[1]) || 0, tot = parseDollar(t[2]);
          if (tot != null && Math.abs((labor + travel + materials + oth) - tot) < 0.02) {
            total = tot; other = oth;
          }
          break;
        }
      ...
      // Emit one item per non-zero category (Labor / Travel / Materials / Other)
      const vals = [labor, travel, materials, other];
      let emitted = 0;
      for (let ci = 0; ci < CATS.length; ci++) {
        if (vals[ci] > 0) {
          out.push({ date: null, reference: ref, description: `${CATS[ci]} Charges`, qty: null, unit_price: null, amount: vals[ci] });
          emitted++;
        }
      }
      // Fallback: push the total if all categories are zero (shouldn't happen)
      if (!emitted) out.push({ date: null, reference: ref, description: desc || 'Service call', qty: null, unit_price: null, amount: total });
    }
    return out;
  })());
