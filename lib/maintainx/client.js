...
function realClient({ token, orgId }) {
  ...
  async function* iterateAssignedWorkOrders({ assigneeId } = {}) {
    ...
    do {
      ...
      qs.append('expand', 'location');
      // Best-effort server-side assignee filter; if the param is wrong the
      // client-side filter below still scopes correctly (just less efficient).
      if (assigneeId) qs.set('assignedTo', String(assigneeId));
  ...
  return { isStub: false, me, getWorkOrder, iterateAssignedWorkOrders, extractMxTime };
}
...
module.exports = { makeClient, extractMxTime, isStubToken };
