...
function realClient({ token, orgId }) {
  ...
  async function* iterateAssignedWorkOrders({ assigneeId } = {}) {
    ...
    do {
      ...
      // Server-side assignee filter — confirmed param name from MaintainX API docs (v0.81.1).
      ...
      for (const wo of items) {
        // Client-side filter as a safety net in case the server doesn't filter perfectly.
        if (!assigneeId || assigneeIds(wo).includes(String(assigneeId))) yield wo;
      }
      // MaintainX returns pagination cursor as 'cursor' (confirmed); fall back to alternatives.
