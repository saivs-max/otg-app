...
function realClient({ token, orgId }) {
  ...
  async function* iterateAssignedWorkOrders({ assigneeId } = {}) {
    ...
    do {
      ...
      // No server-side assignee filter exists; we filter client-side.
      qs.append('expand', 'assignees');
      // v0.82 — Fetch all WO statuses. Without this, MaintainX defaults to
      // returning only open/active WOs, so DONE WOs are never pulled and
      // their Bread status never updates to 'completed'.
      for (const s of ['OPEN', 'IN_PROGRESS', 'ON_HOLD', 'DONE', 'CANCELED', 'SKIPPED']) {
      ...
      // Confirmed field name from live API: nextCursor (integer sequential ID).
