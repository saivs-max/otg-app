...
function realClient({ token, orgId }) {
  ...
  async function* iterateAssignedWorkOrders({ assigneeId } = {}) {
    // v0.82.3 — MaintainX only honours ONE statuses= value when multiple are sent
    // (takes the last, ignores the rest). Iterate per-status to cover all states.
    // expand=assignees adds assignees:[{id,type}] to each list item — required
    // because the base list response omits assignee data entirely.
    const seen = new Set();
    for (const status of ['OPEN', 'IN_PROGRESS', 'ON_HOLD', 'DONE', 'CANCELED', 'SKIPPED']) {
      let cursor = null;
      do {
        ...
        console.log(`[MaintainX] status=${status} page: ${items.length} WOs (assigneeId=${assigneeId})`);
        for (const wo of items) {
          const woId = String(wo.id ?? wo.workOrderId ?? '');
          if (!woId || seen.has(woId)) continue;
          seen.add(woId);
          const ids = assigneeIds(wo);
          const match = !assigneeId || ids.includes(String(assigneeId));
          if (!match) console.log(`[MaintainX] skip WO ${woId} (${wo.status}): assigneeIds=${JSON.stringify(ids)}`);
          if (match) yield wo;
        }
