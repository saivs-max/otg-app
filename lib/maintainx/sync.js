...
async function syncForUserWithOrgKey(db, userId, { clientFactory } = {}) {
  ...
  // Status strings MaintainX uses for completed work orders.
  const DONE_STATUSES = new Set(['DONE', 'COMPLETED', 'COMPLETE', 'CLOSED', 'RESOLVED']);

  try {
    for await (const raw of client.iterateAssignedWorkOrders({ assigneeId })) {
      try {
        // The list endpoint (even with expand=assignees) omits timeEntries[].
        // For completed WOs we need the full single-WO detail to see logged time.
        let effectiveRaw = raw;
        const mxStatus = String(raw.status || '').toUpperCase().replace(/[\s\-]+/g, '_');
        if (DONE_STATUSES.has(mxStatus) && !Array.isArray(raw.timeEntries)) {
          try {
            effectiveRaw = await client.getWorkOrder(raw.id ?? raw.workOrderId);
          } catch (_) { /* use list data on API error */ }
        }
