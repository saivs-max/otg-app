  ...
  if (existing) {
    ...
    db.prepare(`
      UPDATE work_orders SET
        ...
        wo_number = COALESCE(?, wo_number), sub_wo_count = ?, priority = ?,
...
async function syncForUserWithOrgKey(db, userId, { clientFactory } = {}) {
  ...
  const summary = { pulled: 0, created: 0, updated: 0, fetched: 0, laborImported: 0, laborMxWins: 0, laborAppWins: 0, laborNone: 0, errors: [] };
  const integ = db.prepare("SELECT id FROM user_integrations WHERE user_id = ? AND provider = 'maintainx'").get(userId);
  console.log(`[MaintainX] Sync start for user ${userId} (${user.email}) mx_user_id=${assigneeId}`);

  try {
    for await (const raw of client.iterateAssignedWorkOrders({ assigneeId })) {
      summary.fetched++;
      try {
        // The list endpoint omits timeEntries[] AND often omits sequentialId
        // (needed for wo_number). Always fetch the full single-WO detail so every
        // WO gets a correct sequential ID and completed WOs get their time entries.
        let effectiveRaw = raw;
        try {
          effectiveRaw = await client.getWorkOrder(raw.id ?? raw.workOrderId);
        } catch (_) { /* fall back to list data on API error */ }
        const r = processRawWorkOrder(db, userId, client, effectiveRaw);
        summary.pulled++;
        if (r.created) summary.created++; else summary.updated++;
        if (r.labor.direction === 'mx_wins') summary.laborMxWins++;
        else if (r.labor.direction === 'pull') summary.laborImported++;
        ...
        console.error(`[MaintainX] WO ${raw && (raw.id ?? raw.workOrderId)} failed:`, e.message);
        summary.errors.push({ id: raw && (raw.id ?? raw.workOrderId), error: e.message });
      }
    }
    if (integ) {
    ...
    console.log(`[MaintainX] Sync complete for user ${userId} (${user.email}): fetched=${summary.fetched} pulled=${summary.pulled} created=${summary.created} updated=${summary.updated} errors=${summary.errors.length}`);
  } catch (e) {
...
// ─── Catalog sync: locations + assets (v0.83) ────────────────────────────────
// Pulls ALL locations and assets from MaintainX using the org key and upserts
// them into mx_locations / mx_assets. Called on demand and from the daily
// scheduler alongside syncAllUsersWithOrgKey.

function upsertLocation(db, raw, now) {
  const mx_id       = String(raw.id ?? raw.locationId ?? '');
  const name        = raw.name || '';
  const parent_mx_id = raw.parentId != null ? String(raw.parentId) : null;
  // Address may be a string or a nested object depending on API version.
  const addrObj = (raw.address && typeof raw.address === 'object') ? raw.address : null;
  const address  = addrObj ? (addrObj.street || addrObj.line1 || null) : (typeof raw.address === 'string' ? raw.address : null);
  const city     = addrObj?.city  ?? raw.city  ?? null;
  const state    = addrObj?.state ?? raw.state ?? null;
  const zip      = addrObj?.zip   ?? addrObj?.postalCode ?? raw.zip ?? raw.postalCode ?? null;
  const country  = addrObj?.country ?? raw.country ?? null;
  const lat      = (raw.latitude  ?? raw.lat ?? null);
  const lng      = (raw.longitude ?? raw.lng ?? null);

  db.prepare(`
    INSERT INTO mx_locations
      (mx_id, name, parent_mx_id, address, city, state, zip, country, latitude, longitude,
       mx_created_at, mx_updated_at, synced_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(mx_id) DO UPDATE SET
      name         = excluded.name,
      parent_mx_id = excluded.parent_mx_id,
  ...
  `).run(mx_id, name, parent_mx_id, address, city, state, zip, country,
         lat != null ? Number(lat) : null,
         lng != null ? Number(lng) : null,
         raw.createdAt || null, raw.updatedAt || null, now);
}

function upsertAsset(db, raw, now) {
  const mx_id       = String(raw.id ?? raw.assetId ?? '');
  const name        = raw.name || '';
  const loc         = raw.location && typeof raw.location === 'object' ? raw.location : null;
  const location_mx_id = loc ? String(loc.id ?? '') : (raw.locationId != null ? String(raw.locationId) : null);
  const cat         = raw.category && typeof raw.category === 'object' ? raw.category : null;
  const category    = cat ? (cat.name || null) : (typeof raw.category === 'string' ? raw.category : null);

  db.prepare(`
    INSERT INTO mx_assets
      (mx_id, name, description, serial_number, model, manufacturer, category, status,
       location_mx_id, mx_created_at, mx_updated_at, synced_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(mx_id) DO UPDATE SET
      name           = excluded.name,
      ...
      location_mx_id = excluded.location_mx_id,
      mx_updated_at  = excluded.mx_updated_at,
      synced_at      = excluded.synced_at
  `).run(mx_id, name, raw.description || null, raw.serialNumber || null,
         raw.model || null, raw.manufacturer || null, category,
         raw.status || null, location_mx_id || null,
         raw.createdAt || null, raw.updatedAt || null, now);
}

async function syncCatalog(db, { clientFactory } = {}) {
  const client = resolveOrgClient(db, clientFactory);
  const now    = new Date().toISOString();
  const stats  = { locations: 0, assets: 0, errors: [] };

  // Sync locations first (assets reference location IDs).
  try {
    for await (const raw of client.iterateLocations()) {
      try { upsertLocation(db, raw, now); stats.locations++; }
      catch (e) { stats.errors.push({ type: 'location', id: raw?.id, error: e.message }); }
    }
  } catch (e) {
    console.error('[MaintainX] Catalog sync — locations failed:', e.message);
    stats.errors.push({ type: 'locations_list', error: e.message });
  }

  // Sync assets.
  try {
    for await (const raw of client.iterateAssets()) {
      try { upsertAsset(db, raw, now); stats.assets++; }
      catch (e) { stats.errors.push({ type: 'asset', id: raw?.id, error: e.message }); }
    }
  } catch (e) {
    console.error('[MaintainX] Catalog sync — assets failed:', e.message);
    stats.errors.push({ type: 'assets_list', error: e.message });
  }

  // Persist last sync timestamp in settings.
  db.prepare(`
    INSERT INTO settings (key, value, updated_at) VALUES ('mx_catalog_synced_at', ?, ?)
    ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at
  `).run(now, now);

  console.log(`[MaintainX] Catalog sync: ${stats.locations} locations, ${stats.assets} assets`);
  return stats;
}
...
module.exports = {
  ...
  // v0.83 — Catalog sync (locations + assets)
  syncCatalog, upsertLocation, upsertAsset,
};
