...
function writeMxTimeEntry(db, { workOrderId, userId, mxTime, anchorDate }, { skipStaleCleanup = false } = {}) {
  ...
  // (e.g. switched from in-progress estimate to an explicit logged entry).
...
function reconcilePerTechLaborCompleted(db, { workOrderId, ownerId, perTechEntries, anchorDate }) {
  ...
  // 3. Delete stale synthetic entries not in the incoming set
  //    (e.g. a tech was removed from the WO before completion).
  const existing = db.prepare(
    "SELECT id, external_ref FROM time_entries WHERE work_order_id = ? AND source = 'maintainx_sync'"
  ).all(workOrderId);
