...
// Extract the assignee id list from a raw MaintainX WO.
// Confirmed field from live API (2026-07): assigneeIds is a flat array of integer user IDs.
// All other shapes kept as fallbacks for edge cases / older API versions.
function assigneeIds(raw) {
  const out = new Set();
  const push = (v) => { if (v != null) out.add(String(v)); };
  // Primary: flat integer array (confirmed live API shape)
  if (Array.isArray(raw.assigneeIds)) for (const id of raw.assigneeIds) push(id);
  // Fallbacks: expanded assignees array (some expand= responses) and legacy shapes
