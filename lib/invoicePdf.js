...
function drawInvoice(doc, { invoice, tech, lines, by_date, summary }) {
  ...
  // v0.95 — use the DB primary key (always unique) instead of last-4-digits
  // of invoice_number, which collided for same-user same-day-of-month weeks.
  const invNum = String(invoice.id).padStart(5, '0');
  ...
  for (const day of (by_date || [])) {
    ...
    for (const t of allEntries) {
      ...
      // Use sequential WO number if available, else fall back to the ticket
      // portion of external_id (strip MX-RPR- prefix) — never show raw slug.
      const woRef = (() => {
        if (t.wo_number) return (t.source_system === 'maintainx' ? 'MaintainX' : t.source_system === 'freshdesk' ? 'Freshdesk' : 'WO') + ' #' + t.wo_number;
        if (t.external_id) {
          const m = (t.external_id || '').match(/^(MX|FD)-(DPL|RTR|SVC|MNT|RPR)-(.+)$/i);
          const src = t.source_system === 'maintainx' || (t.external_id||'').startsWith('MX-') ? 'MaintainX' : t.source_system === 'freshdesk' || (t.external_id||'').startsWith('FD-') ? 'Freshdesk' : null;
          return src ? `${src} #${m ? m[3] : t.external_id}` : t.external_id;
        }
        return '';
      })();
      const detail =
        `[${fmtMonthDay(t.clock_in)}] ${isDrive ? '[DRIVE]' : `[${(t.work_type||'').toUpperCase()}]`} ${woRef}` +
  ...
  if (mileageDays.length) {
    ...
    doc.text(
      `${tech?.name || ''} · ${invoice.period_start} – ${invoice.period_end} · Rate: $${(mileageDays[0].stops[0]?.rate || 0.725).toFixed(3)} / mile`,
  ...
  doc.text(`Email: ${tech?.invoice_email || tech?.email || ''}` + (tech?.home_phone ? `   Mobile: ${tech.home_phone}` : ''),
