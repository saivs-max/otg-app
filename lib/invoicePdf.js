...
async function generateInvoicePdf(invoiceData) {
  // v0.99 — contactEmail is the company AP/support address the tech can use to
  // reach out with questions; it is sourced from policy/env by the caller and
  // must never be taken from the tech's own profile (which is untrusted input).
  const { invoice, tech, lines, by_date, summary, approvals, tz, contactEmail } = invoiceData;
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.on('error', reject);
      // v1.05 — thread contactEmail through to drawInvoice; it reads it when
      // drawing the footer. Without this every render threw ReferenceError:
      // contactEmail is not defined (regression from the v0.99 footer change).
      drawInvoice(doc, { invoice, tech, lines, by_date, summary, tz, contactEmail });
      doc.end();
    } catch (e) { reject(e); }
  });
}

function drawInvoice(doc, { invoice, tech, by_date, summary, tz, contactEmail }) { // v1.05
  ...
  y += 12;
  // v0.99 — show the company contact email (from policy), not the tech's own email.
  // The tech's invoice_email/email is payment routing info and must not appear here.
  const footerEmail = contactEmail || 'ap@instacart.com';
  const contact = `Email: ${footerEmail}${tech?.home_phone ? `   ·   Mobile: ${tech.home_phone}` : ''}`;
