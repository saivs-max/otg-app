...
//   classifyMxWorkType(workOrder)     → 'deployment' | 'retrofit' | 'maintenance' | 'repair' | null
...
//   - type field: "REACTIVE" → repair, "PREVENTIVE" → maintenance
const WT_KEYWORDS = [
  ...
  { wt: 'maintenance', re: /\b(service|inspection|calibrat|maintenance|check|tune)/i },
];
function classifyMxWorkType(w) {
  ...
  for (const [k, v] of Object.entries(ef)) {
    ...
    if (['service type', 'work type', 'visit type', 'wo type'].includes(key) && typeof v === 'string') {
      ...
      if (/service|inspect|calibrat|tune/.test(lc)) return 'maintenance';
    }
  }
  ...
  if (w.type === 'PREVENTIVE') return 'maintenance';
  return null;
}
